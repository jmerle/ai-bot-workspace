...
const updateSettings = () => {
  const bots = Config.getConfig().bots;
  bots.forEach((bot, id) => {
...
$('#viewer iframe')[0].onload = () => {
  if ($('#viewer iframe').attr('src') !== '') {
    ...
    $('.log.segment[data-tab="bot1-stderr"] > pre').html(matchData.resultFile.stderr[0]);
    $('.log.segment[data-tab="bot2-stderr"] > pre').html(matchData.resultFile.stderr[1]);
...
$('#batch-settings').on('submit', async e => {
  ...
  let bots = Config.getConfig().bots;
  ...
  for (let i = 1; i <= amount; i++) {
    try {
      ...
      let winnerID = matchData.resultFile.game.winner;
